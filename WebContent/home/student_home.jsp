...
String shen=(String)session.getAttribute("shen");
 ...
 session.setAttribute("user", user);
  ...
  kecheng_list=(ArrayList<Kecheng>)request.getAttribute("kecheng_list");
  ...
  session.setAttribute("student_kecheng_list",student_kecheng_list);
  int j=0;
    ...
    <div style="width:100%;height:100%;background:#f8f8f8;">
       ...
       <div style="width:1200px;margin:0 auto;height:175px;border-radius: 3px;background: #fff;">
          <div style="float:left;width:145px;height:145px;padding: 15px 0px 0px 30px;">
             <img src="${pageContext.request.contextPath}/<%=user.getImg() %>" style="width:100%;height:100%;border-radius: 50%;">
          </div>
          <div style="float:left;width:425px;height:135px;padding: 25px 0px 0px 30px;">
               <div style="color: #333;font-size: 16px;padding: 28px 0px 8px;"><%=user.getName() %></div>
               <div style="color: #333;font-size: 12px;padding-bottom: 10px;">西南大学 </div>
               <div style="color: #333;font-size: 12px;padding-bottom: 10px;"><%=user.getZhuanye() %> </div>
               <div style="color: #333;font-size: 12px;padding-bottom: 10px;"><%=user.getNianji() %>级<%=user.getCla() %>班</div>
          </div>
          <div style="float:left;width:285px;height:130px;padding-top: 15px;">
            <div style="height:60px;background: #f8f8f8;padding: 8px 13px 0px;margin-right: 15px;margin-bottom: 10px;"><a href="${pageContext.request.contextPath}/Student_home_mykecheng"><p style="float:left"><span style="background:url()"></span>我的课程</p><p style="float:right;"><%=kecheng_list.size() %></p></a></div>
            <div style="height:60px;background: #f8f8f8;padding: 8px 13px 0px;margin-right: 15px;margin-bottom: 10px;"><a href="${pageContext.request.contextPath}/Student_xiaoxi"><p style="float:left"><span style="background:url()"></span>我的消息</p><p style="float:right;"><%=j %></p></a></div>
          </div>
         <div style="float:left;width:285px;height:130px;padding-top: 15px;">
       ...
       <div style="height:270px;width:1200px;margin:0 auto;padding-bottom: 20px;background: #fff;border-radius: 3px;">
          <div style="padding:5px 0 0 10px;width:300px;border-radius: 3px;height: 31px;border: #dcdcdc solid 1px;">
             <span id="mykecheng_xianshi"><%=NewsService.getKechengBykecheng_id(student_kecheng_list.get(0).getKecheng_id()).getKe_name() %></span>
          ...
          <div id="mykecheng" style="display:none;margin-top:5px;width:300px;border-radius: 3px;border: #dcdcdc solid 1px;">
              ...
              <a href=""><div id="mykecheng<%=map.getId() %>" style="cursor: pointer;height:26px;padding:10px 0px 0 10px"><%=kecheng.getKe_name() %></div></a>
               <%}}else{ %>
       ...
       <div style="min-height:100px;width:1200px;margin:0 auto;padding-bottom: 20px;background: #fff;border-radius: 3px;">
          <div style="height: 60px;padding: 15px 20px 0px;">
             <%if(!kecheng_list.isEmpty()){
	    	    for(Kecheng map:kecheng_list){ 
	    	    Teacher_kecheng teacher_kecheng=NewsService.getTeacher_kechengByKecheng_id(map.getId());
	    	    Teacherusers teacher=UserService.chauserTeacher(teacher_kecheng.getTeacher_xuehao());
	    	    %>
             <div style="text-align:center;float:left;width:200px;height:300px;margin:15px;">
               <img src="${pageContext.request.contextPath}/<%=map.getImg() %>" style="width:200px;height:200px">
               <div style="">
                 <p><%=map.getKe_name() %></p>
